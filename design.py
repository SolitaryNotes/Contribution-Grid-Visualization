...
ax.text(legend_x, legend_y - 6, 'Muted', ha='left', va='bottom',
        fontsize=10, color=COLOR_LEGEND_TEXT, fontweight='medium', fontfamily='sans-serif')

# 图例方格的大小与贡献图方格保持一致 (CELL_SIZE)
for i, color in enumerate([COLOR_EMPTY, COLOR_RECENT, COLOR_MEETING]):
    rect = patches.FancyBboxPatch(
        (legend_x + 35 + i * (CELL_SIZE + GAP + 5), legend_y - 6), 
        CELL_SIZE, CELL_SIZE,
        boxstyle=f"round,pad=0.2,rounding_size=1.5",
        facecolor=color, edgecolor='none'
    )
...
ax.text(legend_x + 35 + 3 * (CELL_SIZE + GAP + 5), legend_y - 6, 'Bright', ha='left', va='bottom',
        fontsize=10, color=COLOR_LEGEND_TEXT, fontweight='medium', fontfamily='sans-serif')

# ================= 输出 SVG =================
ax.axis('off')
plt.tight_layout()
plt.savefig('final_contribution_grid_v3.svg', format='svg', dpi=300, bbox_inches='tight')
print("✅ 最终完美版已生成：final_contribution_grid_v3.svg")